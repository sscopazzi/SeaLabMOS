...
void setup() {
  ...
  if (salinityBool && displayBool) { display.println("Salinity init");   display.display(); }
  ...
  Wire.begin();
  // if (bar02Bool) {Wire.setClock(100000); }   // <-- force 100 kHz for reliability at any sys clock
  delay(100);
  ...
  if (bar02Bool || bar30Bool || bar100Bool) { brPressureSetup(); 
  if (displayBool) { display.println("brPressure init"); display.display(); } }  
  delay(200);

  // SETUP FAST TEMP
  if (brFastTempBool) { brFastTempSetup(); 
  if (displayBool) { display.println("brFastTemp init"); display.display(); } }  
  delay(200);

  if (dallasTempBool && displayBool ) { display.println("dallasTemp init"); display.display(); }
  if (thermTempBool  && displayBool ) { display.println("thermTemp init"); display.display(); }
  if (pt100Bool) { pt100Setup(); }
  if (pt100Bool && displayBool)    {display.println("PT100 init"); display.display(); }
  ...
  rtc.clearAlarm(1); delay(2); // tiny debounce delay for some DS3231 boards, not 100% sure if needed
  rtc.clearAlarm(2); delay(2); // tiny debounce delay for some DS3231 boards, not 100% sure if needed

  pinMode(RTC_INTERRUPT_PIN, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(RTC_INTERRUPT_PIN), rtcWakeISR, FALLING);

  if (displayBool) { display.println("RTC init"); display.display(); }
  delay(200);
...
void runMode1and2() {
  ...
  if (displayBool == true) {
    display.clearDisplay();
    display.setCursor(0, 0);
    display.println("Salinity powered on"); 
  ...
  if (!rtcAlarmFired) {
    delay(50);
    // Returning here is okay because it's inside a dedicated function
    return;
  }
...
void runMode4() {
  ...
  if (rtcAlarmFired) {
    ...
    if (serialDisplay) {
      Serial.print("BPR: ");  Serial.print(bprSampling ? "START " : "STOP "); Serial.print("sampling; next boundary @ ");
